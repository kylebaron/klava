...
So far, this is  a __parameter management system__.  We can name and set initial values
for parameters in a model.  We can also specify a transformation
for each parameter: the value is transformed to a different scale for estimation and 
it can be transformed back when either getting a prediction or after the optimization is finished.  Also, `parset` objects allow you to fix parameters in the optimization.
...
Grafting back into the `parset` object gets us untransformed (by default)
```{r}
...
In the `pred` function below
  ...
  1. Graft the estimated proposed by the optimzer back into the `parset` object
...
1. Pass the `parset` object into the prediction function so we can graft the estimates back in
```{r}
...
back into the `parset` and take a look.
```{r}
est <- graft(p,fit$par)
